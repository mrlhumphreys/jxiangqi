...
class SquareSet {
  ...
  findJiangForPlayer(playerNumber) {
    return this.occupiedByPiece('jiang').occupiedByPlayer(playerNumber).first();
  }

  threatenedBy(playerNumber, gameState) {
    let destinations = this.occupiedByPlayer(playerNumber).excludingPiece('jiang').map((s) => {
